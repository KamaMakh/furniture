<template>
  <div>
    <table class="table" v-if="warehouse">
        ...
        <td>
          <span @click="hideAll" class="icon" style="cursor: pointer">
            <svg width="20" height="17" viewBox="0 0 20 17" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M3 9H0L10 0L20 9H17V17H11V11H9V17H3V9ZM15 7.19L10 2.69L5 7.19V15H7V9H13V15H15V7.19Z" fill="#C4C5C4"/>
            </svg>
          </span>
          ...
          <span class="d-flex justify-content-end">
            ...
            <span class="setting-icon mr-1 ml-0">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M5.5 5.5V9.5H7.5V5.5H10L6.5 2L3 5.5H5.5Z" fill="#EEEEEE"/>
                <path d="M12.5 11.5L12.5 7.5L10.5 7.5L10.5 11.5L8 11.5L11.5 15L15 11.5L12.5 11.5Z" fill="#EEEEEE"/>
              </svg>
            </span>
            <span class="setting-icon mr-1 ml-0">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M16 0H2C0.89 0 0 0.9 0 2V16C0 17.1 0.89 18 2 18H16C17.1 18 18 17.1 18 16V2C18 0.9 17.1 0 16 0ZM14 10H10V14H8V10H4V8H8V4H10V8H14V10Z" fill="#C4C4C4"/>
              </svg>
            </span>
          {{ $t("order") }} / {{ $t("warehouse") }}
          </span>
      ...
      <tbody>
      <tr v-for="(item, key) in warehouse.rows" :key="key" :class="{odd: key%2 === 0 || key === 0, hidden: warehouse.groups && (!item.isParent && warehouse.groups[item.parentName+item.parentId]) || hideAllRows, parent: item.isParent}">
        ...
        <td v-if="item.isParent">
          <span class="d-flex align-items-center justify-content-end">
            <span class="icon mr-0" style="cursor: pointer" @click="toggleRows(item.name+item.id)">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <td v-else :style="{color: item.statusType == 1 ? '#FF4081' : item.statusType == 2 ? '#154E85' : item.statusType == 3 ? '#9B51E0' : '#00670A'}">
          {{ item.status }}
        </td>
        <td></td>
        <td></td>
